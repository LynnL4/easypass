...
class role extends \MySQLPDO
{
    ...
    {
        return $this->fetchAll("SELECT tbl_community_admin.id,tbl_user.user_name,tbl_account.login_name,tbl_community.community_name,tbl_community_admin.account_id, tbl_user.avaurl
    ...
    public function addItemToUser($user_name,$email) //将信息添加到用户信息列表
    {
        ...
        if($this->exec("INSERT INTO `tbl_user`(user_name,email) VALUES (:user_name,:email)",$data)){
    ...
    public function Reset($id)
    {
        ...
        return $this->exec("UPDATE `tbl_account` SET `password`='000000' WHERE (`id`=$id);");
    }
}