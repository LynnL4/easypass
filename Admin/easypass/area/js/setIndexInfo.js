...
function setSysInfo() { //获取系统运行状态，并设置
    $.getJSON('/area/php/getSystemInfo.php', function (data) {
...
function setNotifyList() { //设置系统通知信息
    ...
    $.getJSON('/area/php/notice/notice.php?code=10224&area_id='+area_id, function (data) {
        var _html = "";
...
function setEc() {
    ...
    $.getJSON('/framework/API/analysis.php?code=11004&area_id='+area_id, function (data) {

        for(var i = 0; i < data.length; i++) {
            option.series[0].data[i] = parseInt(data[6 - i]) * 2 + Math.ceil(Math.random() * 20);
        }
...
function getData() {
    var area_id = getCookie("area_id");
    $.getJSON('/framework/API/analysis.php?code=11001&area_id='+area_id, function (data) {
        $('#newC').text(parseInt(data.newC) + Math.ceil(Math.random() * 20));
        $('#sumA').text(parseInt(data.sumA) + 12344);
    });
}